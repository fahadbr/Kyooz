...
internal enum TweakViewData {
	...
	init<T: TweakableType>(type: TweakViewDataType, value: T, defaultValue: T, minimum: T?, maximum: T?, stepSize: T?) {
		...
		case .integer:
			let clippedValue = clip(value as! Int, maximum as? Int, minimum as? Int)
		...
		case .cgFloat:
			let clippedValue = clip(value as! CGFloat, maximum as? CGFloat, minimum as? CGFloat)
		...
		case .double:
			let clippedValue = clip(value as! Double, maximum as? Double, minimum as? Double)
