...
internal final class TweakPersistency {
	...
	internal func currentValueForTweak<T where T: SignedNumber>(_ tweak: Tweak<T>) -> T? {
		if let currentValue = persistedValueForTweakIdentifiable(AnyTweak(tweak: tweak)) as? T {
				...
				return clip(currentValue, tweak.maximumValue, tweak.minimumValue)
		}
...
private final class TweakDiskPersistency {
	...
	private static func fileURLForIdentifier(_ identifier: String) -> URL {
		return try! FileManager().urlForDirectory(.documentDirectory, in: .userDomainMask, appropriateFor: nil, create: true)
			.appendingPathComponent("SwiftTweaks")
			.appendingPathComponent("\(identifier)")
			.appendingPathExtension("db")
	}

	private let queue = DispatchQueue(label: "org.khanacademy.swift_tweaks.disk_persistency", qos: DispatchQueue.Attributes.serial)
	...
	func saveToDisk(_ data: TweakCache) {
		self.queue.async {
			...
			try? nsData.write(to: self.fileURL, options: [.dataWritingAtomic])
		}
	}
